...
(function($, window, document, underfined) {
    ...
    $(window).on("scroll", function() {
        ...
        } else if ($("header").hasClass("custom")) {} else {
    ...
    function moveMenu(direction) {
        ...
        $(".prague-header.full nav").css(
            "transform",
            "translateX(-" + show_level * 100 + "%)"
        );
    ...
    function pageHeightCalculate() {
        if ($(".page-calculate.fullheight").length) {
            var pageCalculate = $(".page-calculate.fullheight"),
                pageCalculateContentHeight = $(
                    ".page-calculate.fullheight .page-calculate-content"
                ).outerHeight(true);
    ...
    function topBannerHeight() {
        ...
        var footerH =
            $(".prague-footer").not(".prague-footer.modern").outerHeight() || 0;
        ...
        $(".full-height-window-min-hard").css(
            "min-height",
            windowH - offsetTop + "px"
        );
        $(".full-height-window-hard").css("height", windowH - offsetTop + "px");
        $(".full-height-window-head-foot").css(
            "height",
            windowH - offsetTop - footerH + "px"
        );
        $(".middle-height-window-hard").css(
            "height",
            (windowH - offsetTop) * 0.8 + "px"
        );
    }
    ...
    function startBtnAnimate() {
        ...
        if (btnCreative.length) {
            for (var i = 0; i < btnCreative.length; i++) {
                if (
                    $(window).scrollTop() >=
                    $(btnCreative[i]).offset().top - $(window).height() * 1.4
                ) {
    ...
    function startFigureAnimate() {
        ...
        if (figureCreative.length) {
            for (var i = 0; i < figureCreative.length; i++) {
                if (
                    $(window).scrollTop() >=
                    $(figureCreative[i]).offset().top - $(window).height() * 0.9
                ) {
    ...
    function initSwiper() {
        ...
        $(".swiper-container").each(function(index) {
            ...
            var paginationType = that.attr("data-pagination-type") ?
                that.attr("data-pagination-type") :
                "bullets";
            var autoPlayVar = parseInt(that.attr("data-autoplay"), 10);
            var autoHeightVar = parseInt(that.attr("data-autoheight"), 10);
            var centerVar = $.isNumeric(that.attr("data-center")) ?
                parseInt(that.attr("data-center"), 10) :
                true;
            ...
            var mouseVar = $.isNumeric(that.attr("data-mouse")) ?
                parseInt(that.attr("data-mouse"), 10) :
                true;
            ...
            swipers[sliderIndex] = new Swiper("." + sliderIndex, {
                    ...
                    if ($(".banner-slider-wrap.andra").length) {
                        if (totalSlides < 10) {
                            $(".banner-slider-wrap.andra")
                                .find(".pag-wrapper")
                                .addClass("total-less10 current-less10");
                        }
                    }
                },
                onSlideChangeEnd: function(swiper) {
                    var totalSlides = that.find(
                        $(".swiper-slide:not(.swiper-slide-duplicate)")
                    ).length;
                ...
                onSlideChangeStart: function(swiper) {
                    var activeIndex =
                        loopVar == 1 ? swiper.realIndex : swiper.activeIndex;
                ...
                paginationBulletRender: function(swiper, index, className) {
                    if ($(".testimonials-swiper").length) {
                        ...
                        var newIndexAuthor =
                            $(swiper.slides[newIndex]).find(".testimonials-author").html() ||
                            "Happy Customer";
                        return (
                            '<span class="' + className + '">' + newIndexAuthor + "</span>"
                        );
                    }
                },
            });
    ...
    function comingSoonValue() {
        if ($(".prague-coming-time-wrapper .coming-time-item").length) {
            var comingSoonElements = $(
                ".prague-coming-time-wrapper .coming-time-item"
            );
    ...
    $(".prague_filter_link_wrapper .filter-btn").on("click", function(e) {
        ...
        $(
            ".prague_services, .prague_books, .prague_exhibition_grid, .prague_grid, .prague_masonry"
        ).isotope({
    ...
    /* TIMELINE SORTING */
    /**********************************/
    $(".project-time-list-header .time-list-header-col span").on(
        "click",
        function(e) {
            $(this)
                .parent(".time-list-header-col")
                .addClass("active")
                .siblings()
                .removeClass("active");
        ...
        setTimeout(function() {
            var timelineWrapper = $(".prague_timeline"),
                timelineContentWrapperHeight = $(
                    ".project-timeline-content-wrapper"
                ).height();
    ...
    $(".prague_timeline .project-timeline-item").on("click", function(e) {
        ...
        var $this = $(this),
            ...
            $gallery_row = $(
                ".timeline-img-item[data-unique-key=" + $this.data("post-key") + "]"
            ),
    ...
    $(".prague_timeline .project-timeline-item .time-item-link").on(
        "click",
        function(e) {
    ...
    $(".timeline-img-item").on("mouseenter", function() {
        ...
        $(".project-timeline-item[data-post-key=" + data_unique_key + "]").addClass(
            "active_hover"
        );
    });
 ...
 */
            ...
            $("html, body")
                ...
                .animate({
                    ...
                    function() {
                        ...
                        $(scrollSelector + '[data-id="' + currentAnchor + '"]').addClass(
                            active_class
                        );
                    }
                );
    ...
    function setScrollAnchor() {
        if (!scrollFlag) {
            ...
            for (var p in contentTop) {
                if (
                    contentTop[p].top <= scrollPositionTop &&
                    contentTop[p].bottom > scrollPositionTop &&
                    currentAnchor != p
                ) {
                    $(scrollSelector).removeClass(active_class);
                    if (history.pushState) {
    ...
    function initProjectDetailSlider() {
        ...
        if (thumb.length) {
            ...
            var verticalSwipingVar = parseInt(
                thumb.attr("data-vertical-swiping"),
                10
            );
    ...
    function scrollCenterProjectList() {
        ...
        if (projectList.length) {
            for (var i = 0; i < projectList.length; i++) {
                if (
                    $(window).scrollTop() >=
                    $(projectList[i]).offset().top - $(window).height() * 0.5
                ) {
                    $("html, body").animate({
    ...
    $(".prague_categories_btn .categories_btn_up").on("click", function() {
        var prevCategory = $(this)
            .closest(".prague-categories-outer")
            .prev(".prague-categories-outer")
            .offset().top;
    ...
    $(".prague_categories_btn .categories_btn_down").on("click", function() {
        var nextCategory = $(this)
            .closest(".prague-categories-outer")
            .next(".prague-categories-outer")
            .offset().top;
    ...
    function pragueGrid() {
        var block =
            ".prague_services, .prague_books, .prague_media, .prague_exhibition_grid, .prague_grid";
    ...
    var Parallax = {
        ...
        initialize: function() {
            ...
            $(".js-detail-parallax-item-bg").each(function(i, cover) {
                ...
                $clone
                    .data("source", $cover)
                    .appendTo(".project-detail-parallax-cover")
                    .show();
        ...
        update: function() {
            ...
            $(".project-detail-parallax-cover .detail-parallax-item-bg").each(
                function(i, cover) {
                    var $cover = $(cover),
                        ...
                        progress2 =
                        (latestKnownScrollY - parallax2.start) /
                        (parallax2.end - parallax2.start);
    ...
    $(".protect-page").on("submit", function(e) {
        ...
        if ($("#pwbox-1234").val() == "enter") {
            ...
            $(".prague-protected-content p").html(
                "<p>(Please, add the password)</p>"
            );
        } else {
            $(".prague-protected-content p").html(
        ...
        if (pageNum <= max) {
            $.ajax({
                ...
                success: function(data) {
                    var html = $(data)
                        .find('[data-unique-key="' + uniqueKey + '"]')
                        .find(".js-load-more-block")
                        .children(".js-filter-simple-block");
                    var $items = $(html);
                    ...
                    if (
                        $container.hasClass("prague-blog-grif-outer") ||
                        $container.hasClass("prague_blog")
                    ) {
                        // console.log('second');
    ...
    $(".woocommerce-tabs").on("click", ".tabs-title", function() {
        ...
        if (
            $(thisItem).siblings(".panel").length > 0 &&
            $(thisItem).siblings(".panel").css("display") === "none"
        ) {
            $(siblings).slideDown(400, function() {});
            $(thisParent)
                .addClass("active")
                .siblings(".tabs-item-wrapp")
                .removeClass("active")
                .children(".panel")
    ...
    if (wrapper.length) {
        $("body").on(
            "click",
            ".custom-product-quantity .q_inc, .custom-product-quantity .q_dec",
            function(e) {
                e.preventDefault();
        ...
        if ($(".services.tile").length) {
            $(".services.tile .item-first").each(function() {
                ...
                if ($(window).width() > 991) {
                    element
                        .find(".img-wrap")
                        .css("height", elementFirstH - textHeight - 30 + "px");
                } else {
    ...
    $(".clicked24").on("click", function() {
        ...
        $(".firstImg").attr(
            "src",
            "img/wooden-nightstand/shop-img-1-gallery-1.jpg"
        );
        $("figure a").attr(
            "href",
            "img/wooden-nightstand/shop-img-1-gallery-1.jpg"
    ...
    $(".clicked34").on("click", function() {
        ...
        $(".firstImg").attr(
            "src",
            "img/wooden-nightstand/shop-img-1-gallery-2.jpg"
        );
        $("figure a").attr(
            "href",
            "img/wooden-nightstand/shop-img-1-gallery-2.jpg"
    ...
    $(".clicked25").on("click", function() {
        ...
        $(".firstImg").attr(
            "src",
            "img/modern-end-table-2/shop-img-7-gallery-2.jpg"
        );
        $("figure a").attr(
            "href",
            "img/modern-end-table-2/shop-img-7-gallery-2.jpg"
    ...
    $(".clicked35").on("click", function() {
        ...
        $(".firstImg").attr(
            "src",
            "img/modern-end-table-2/shop-img-7-gallery-1.jpg"
        );
        $("figure a").attr(
            "href",
            "img/modern-end-table-2/shop-img-7-gallery-1.jpg"
